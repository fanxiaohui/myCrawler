...
const char* const filelinkblock::BASE_DATA_PATH = "./";
...
filelinkblock::filelinkblock(const char* path, const char* name, bool readonly)
{
    ...
    if ( readonly && 0 != access(BASE_OFFSET_PATH, F_OK))
    {
...
void filelinkblock:: check_and_repaire()
{
    ...
    else
    {
        ...
        int step =(int)(readlen/sizeof(uint32_t));
        while (--step >= 0)
        {
...
int filelinkblock::__write_message(flb_basic_head& basic_head, const char* buff, const uint32_t buff_size)
{
    ...
    if (cur_offset + wlen > FILE_MAX_SIZE)
    {
        ...
        snprintf(last_file_name, sizeof(last_file_name), "%s/%s.%d", m_flb_path, m_flb_name, m_last_file_no);
        m_flb_w_fd = newfile(last_file_name);
        __write_message(basic_head, buff, buff_size); // digui 
    }
    else
    {
        ...
        myhead.piece1_len = basic_head.piece1_len;
        myhead.timestamp  = (uint32_t)time(NULL);
        myhead.log_id     = basic_head.log_id;
        myhead.queue_type = basic_head.queue_type;
        myhead.event_type = basic_head.event_type;
...
int filelinkblock::write_message(flb_basic_head& basic_head, const char* buff, const uint32_t buff_size)
{
    assert (buff != NULL && buff_size > 0);
    pthread_mutex_lock(&m_mutex);
    int ret = __write_message(basic_head, buff, buff_size);
...
int filelinkblock::read_message(flb_basic_head& basic_head, char* buff, const uint32_t buff_size)
{
    assert (buff != NULL && buff_size > 0);
    struct file_link_block_head myhead;
    ...
    while(1)
    {
        ...
        else
        {
            ...
            MySuicideAssert(myhead.magic_num == MAGIC_NUM);
            uint32_t stepsize = ((myhead.block_size + 3 ) >> 2 ) << 2;
            // TODO 应该判断内存是否够用，不然这里太粗暴
            MySuicideAssert (stepsize < buff_size);
            MySuicideAssert(BLOCK_MAX_SIZE > myhead.block_size + sizeof(file_link_block_head));
            MySuicideAssert(stepsize == (uint32_t)readn(m_flb_r_fd, buff, stepsize));
            m_flb_read_offset = (uint32_t)lseek(m_flb_r_fd, 0, SEEK_CUR);
            basic_head.log_id     = myhead.log_id;
            basic_head.block_id   = myhead.block_id;
            basic_head.piece1_len = myhead.piece1_len;
            basic_head.queue_type   = myhead.queue_type;
            basic_head.event_type   = myhead.event_type;
            basic_head.file_no    = m_flb_read_file_no;
            // 校验block_id即可，就不用计算check_num了
