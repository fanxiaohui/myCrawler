...
void host_links::init(const char* visit_interval_config_path)
{
    ...
    for (uint32_t i = 0; i < keynames.size(); i++)
    {
        ...
        DEBUG("host[%s] max_crawling_number[%u] pick_url_interval_ms[%u]", key.c_str(), crawl_stat.max_crawling_number, pick_url_interval_ms);
...
link_info_t* host_links::get_link(const string& host)
{
    ...
    {
        ROUTN("host[%s] need slow down, queue_length[%u] crawling_number[%u] max_crawling_num[%u]", host.c_str(), crawl_stat_pre.queue_length, crawl_stat_pre.crawling_number, crawl_stat_pre.max_crawling_number);
        return NULL;
    }
