...
int crawl_the_page(event_base* base, evdns_base* dnsbase, link_info_t* plink_info)
{
    ...
    struct evhttp_request * my_evhttp_request = evhttp_request_new (request_cb, my_request_context);
...
static void start_crawling(int fd, short _event, void *arg)
{
    // 回调函数被唤醒，某个站点需要抓取了
    ...
    switch(notify_context->status)
    {
        case READ_NOTIFICATION_LENGTH:
            MySuicideAssert(sizeof(notify_context->host_length) == read(fd, &(notify_context->host_length), sizeof(notify_context->host_length)));
